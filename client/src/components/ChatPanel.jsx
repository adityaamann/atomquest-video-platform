  ...
  if (isImage) {
    return (
      <a href={url} target="_blank" rel="noopener noreferrer">
        <img src={url} alt={msg.content || 'image'} className="max-w-full max-h-48 rounded-lg mt-1 border border-slate-200" />
      </a>
    )
  }
  return (
...
export default function ChatPanel({ messages, onSend, currentUser, socketRef, sessionId, typingPeer }) {
  ...
  async function handleFileChange(e) {
    ...
    try {
      ...
      const { data } = await api.post('/api/files/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-slate-200 rounded-xl overflow-hidden">
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="p-3 border-t border-slate-100 flex gap-2 items-center bg-slate-50">
        ...
        <button type="button" disabled={uploading} onClick={() => fileInputRef.current?.click()}
          className="text-slate-400 hover:text-slate-600 disabled:opacity-40 transition-colors shrink-0 p-1">
          {uploading ? (
