...
export default function VideoPlayer({ stream, muted = false, label, className = '', audioEnabled = true, videoEnabled = true }) {
  ...
  return (
    <div className={`relative rounded-xl overflow-hidden bg-gray-800 ${className}`}>
      {stream ? (
        <video
          ref={videoRef}
          autoPlay
      ...
      {/* Camera-off overlay — shown when video stream exists but participant turned camera off */}
      {stream && !videoEnabled && (
      ...
      {label && (
        <div className="absolute bottom-2 left-2 flex items-center gap-1.5">
          ...
          {!audioEnabled && (
            <div className="bg-red-600/80 text-white text-xs px-1.5 py-1 rounded flex items-center gap-0.5">
